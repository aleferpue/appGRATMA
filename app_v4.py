...
class GratmaApp:
    ...
    def _build_meas_params(self) -> None:
        ...
        # adds the stabilization/settle timings and the number of sequences.
        self._params_random = ttk.LabelFrame(self._meas_params_frame,
                                             text="Randomised Parameters", padding=10)
        ...
        self._rnd_total_seq = self._entry_row(self._params_random, "Sequences (total):", 4, 2)
        Label(self._params_random,
              text="Procedure: connect all channels @0 V → wait (stabilization) →\n"
                   ...
                   "for every sensor, N sequences total. The 1st sequence is discarded;\n"
                   "the rest are exported (one CSV per sensor & repetition).\n"
    ...
    def _on_start_measurement(self) -> None:
        ...
        try:
            ...
            if meas_type == "iv_parametric":
                ...
                total_seq = int(self._rnd_total_seq.get())
                if stab_s < 0 or settle_s < 0:
                    raise ValueError("Wait times must be >= 0")
                if total_seq < 2:
                    raise ValueError("Sequences must be >= 2 (the first one is discarded)")
                ...
                self._run_async(self._meas_iv_random_worker,
                                ...
                                stab_s, settle_s, total_seq, random_mode)
    ...
    def _meas_iv_random_worker(self, vs, vg_start, vg_end, vg_step, sensors_list,
                               reverse, stab_s, settle_s, total_seq, random_mode) -> None:
        self._log_write(
            f"I-V Randomised: VS(Vd)={vs}mV  VG={vg_start}→{vg_end}mV  step={vg_step}mV  "
            f"sensors={sensors_list}  sequences={total_seq} (1 discarded)  "
            f"random={'on' if random_mode else 'off'}", CYAN)
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        # 4-6. Run the sequences. The first one is discarded; the rest are kept
        # and renumbered rep 1..(total_seq-1).
        kept: list[dict] = []
        for seq in range(1, total_seq + 1):
            ...
            discard = (seq == 1)
            tag = "discarded" if discard else f"kept as rep {seq - 1}"
            self._log_write(
            ...
            for sensor in order:
                ...
                # the sensor we selected and the repetition index (0 = discarded).
                for r in pts:
                    r["sensor"] = sensor
                    r["rep"] = seq - 1
                if not discard:
        ...
        meta = {
            "timestamp": timestamp, "vs_mv": vs, "vg_start_mv": vg_start,
            "vg_end_mv": vg_end, "vg_step_mv": vg_step, "reps": total_seq - 1,
            "reverse": reverse,
        }
        self._rq.put(("meas_done", {
            "type": "iv_random", "records": kept, "result": None, "meta": meta,
            "n_sensors": len(sensors_list), "n_reps": total_seq - 1,
        }))
